...
public class CapacitorGooglePay extends Plugin {
    ...
    public void configure(PluginCall call) {
        ...
        this.paymentsClient = Wallet.getPaymentsClient(
                getActivity(),
                new Wallet.WalletOptions.Builder().setEnvironment(useProdEnvironment ? WalletConstants.ENVIRONMENT_PRODUCTION : WalletConstants.ENVIRONMENT_TEST).build()
        );
    ...
    public void loadPaymentData(PluginCall call) {
        ...
        if (request != null) {
            final PluginCall callContext = call;
            final Task<PaymentData> paymentDataTask = this.paymentsClient.loadPaymentData(request);

            paymentDataTask.addOnFailureListener(getActivity(), new OnFailureListener() {
                @Override
